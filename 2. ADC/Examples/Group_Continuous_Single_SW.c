/**********************************************************
* File Name   : Group_Continuous_Single_SW.c
* Module      : ADC
* Description : This file is part of the ADC example for STM32F103C8T6
* Details     : This file demonstrates the use of ADC in continuous single mode for a group of channels with software trigger.
* Version     : 1.0.0
* Date        : 26/06/2025
...
volatile uint16_t buffer[2] = {0}; // Buffer for DMA ADC values, marked volatile for debugging & watch
...
int main()
{
    ...
    while (1)
    {
        ...
        if (dma_conversion_complete)
        {
            // Reset the flag for next conversion
            dma_conversion_complete = 0;
            
            // Process the ADC values that were updated by DMA
            voltageInMv[0] = (buffer[0] * 3300) / 4095;  // Channel 0 (PA0) in millivolts
            voltageInMv[1] = (buffer[1] * 3300) / 4095;  // Channel 1 (PA1) in millivolts
            
            // Keep adcValue updated for backward compatibility
            adcValue = buffer[0];
...
void DMA_Config(void)
{
    ...
    dma.DMA_BufferSize = 2;                     // Number of ADC channels to read
...
void Adc_Config(void)
{
    ...
    ADC_RegularChannelConfig(ADC1, ADC_Channel_0, 2, ADC_SampleTime_28Cycles5); // ADC Channel 1 (PA1)
